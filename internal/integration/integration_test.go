...
func testImplicitIndexes(t T, db *sql.DB) {
	const (
		...
		ddl  = `create table implicit_indexes(
			"id" integer NOT NULL GENERATED BY DEFAULT AS IDENTITY,
			PRIMARY KEY ("id")
			"c1" int unique, "c2" int unique, unique("c1","c2"), unique("c2","c1"))`
	)
	t.dropTables(name)
	_, err := db.Exec(ddl)
	...
	desired := schema.NewTable(name).
		...
		).SetPrimaryKey(schema.NewUniqueIndex("primary").AddParts(schema.NewIndexPart().SetColumn(id)))
...
func ensureNoChange(t T, tables ...*schema.Table) {
	realm := t.loadRealm()
	require.Equal(t, len(tables), len(realm.Schemas[0].Tables))
	for i := range tables {
