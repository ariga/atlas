...
func TestDestructive_SkipTemporaryTable(t *testing.T) {
	var (
		report *sqlcheck.Report
		pass   = &sqlcheck.Pass{
			Dev: &sqlclient.Client{},
...
func TestDestructive_DropSchema(t *testing.T) {
	var (
		report *sqlcheck.Report
		pass   = &sqlcheck.Pass{
			Dev: &sqlclient.Client{},
...
func TestDestructive_Register(t *testing.T) {
	sqlcheck.Destructive.Register("mysql", func(ctx context.Context, p *sqlcheck.Pass) (diags []sqlcheck.Diagnostic, _ error) {
		for _, cs := range p.File.Changes {
			for _, c := range cs.Changes {
				_, ok := c.(*schema.ModifySchema)
				if !ok {
					continue
				}
				// A fake driver-level diagnostic.
				diags = append(diags, sqlcheck.Diagnostic{Text: "fake diagnostic", Pos: cs.Pos})
			}
		}
		return
	})
	...
	require.NoError(t, err)
	require.Len(t, report.Diagnostics, 2)
	require.Equal(t, "fake diagnostic", report.Diagnostics[0].Text)
	require.Equal(t, `Dropping non-virtual column "c"`, report.Diagnostics[1].Text)
}
