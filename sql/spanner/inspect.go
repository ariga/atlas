...
func (i *inspect) InspectRealm(ctx context.Context, opts *schema.InspectRealmOption) (*schema.Realm, error) {
	...
	if len(schemas) > 1 {
		return nil, fmt.Errorf("spanner: multiple database files are not supported by the driver. got: %d", len(schemas))
	}
...
func (i *inspect) InspectSchema(ctx context.Context, name string, opts *schema.InspectOptions) (*schema.Schema, error) {
	...
	if len(schemas) == 0 {
		return nil, &schema.NotExistError{
			Err: fmt.Errorf("spanner: schema %q was not found", name),
		}
	}
	s := schemas[0]
...
func (i *inspect) columns(ctx context.Context, t *schema.Table) error {
	...
	if err != nil {
		return fmt.Errorf("spanner: querying %q columns: %w", t.Name, err)
	}
	defer rows.Close()
	for rows.Next() {
...
func (i *inspect) addColumn(t *schema.Table, rows *sql.Rows) error {
	...
	// 3 for STORED columns. See: spanner/pragma.c#spanner3Pragma.
	if hidden.Int64 >= 2 {
...
func (i *inspect) indexes(ctx context.Context, t *schema.Table) error {
	...
	if err != nil {
		return fmt.Errorf("spanner: querying %q indexes: %w", t.Name, err)
	}
	if err := i.addIndexes(t, rows); err != nil {
		return fmt.Errorf("spanner: scan %q indexes: %w", t.Name, err)
	}
...
func (i *inspect) indexInfo(ctx context.Context, t *schema.Table, idx *schema.Index) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&name, &desc); err != nil {
			return fmt.Errorf("spanner: scanning index names: %w", err)
		}
		...
		default:
			return fmt.Errorf("spanner: column %q was not found for index %q", name.String, idx.Name)
		}
...
func (i *inspect) fks(ctx context.Context, t *schema.Table) error {
	...
	if err != nil {
		return fmt.Errorf("spanner: querying %q foreign-keys: %w", t.Name, err)
	}
	if err := i.addFKs(t, rows); err != nil {
		return fmt.Errorf("spanner: scan %q foreign-keys: %w", t.Name, err)
	}
...
func (i *inspect) tables(ctx context.Context, opts *schema.InspectOptions) ([]*schema.Table, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("spanner: querying schema tables: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&name, &stmt); err != nil {
			return nil, fmt.Errorf("spanner: scanning table: %w", err)
		}
...
type (
	...
	// https://www.spanner.org/autoinc.html
	AutoIncrement struct {
		schema.Attr
		// Seq represents the value in spanner_sequence table.
		// i.e. https://www.spanner.org/fileformat2.html#seqtab.
		//
	...
	// See: https://spanner.org/withoutrowid.html
	WithoutRowID struct {
		schema.Attr
	}

	// IndexPredicate describes a partial index predicate.
	// See: https://www.spanner.org/partialindex.html
	...
	// See: https://www.spanner.org/pragma.html#pragma_index_list
	IndexOrigin struct {
		schema.Attr
		O string
	}
...
func defaultExpr(x string) schema.Expr {
	switch {
	// Literals definition.
	// https://www.spanner.org/syntax/literal-value.html
...
// CREATE statement, according to https://www.spanner.org/syntax/column-constraint.html.
...
func autoinc(t *schema.Table) error {
	...
	if !ok {
		return fmt.Errorf("spanner: column %q was not found for AUTOINCREMENT", matches[1])
	}
	if t.PrimaryKey == nil || len(t.PrimaryKey.Parts) != 1 || t.PrimaryKey.Parts[0].C != pkc {
		return fmt.Errorf("spanner: unexpected primary key: %v", t.PrimaryKey)
	}
	inc := &AutoIncrement{}
...
func setGenExpr(t *schema.Table, c *schema.Column, f int64) error {
	...
	if len(idx) != 1 || len(idx[0]) != 2 {
		return fmt.Errorf("spanner: generation expression for column %q was not found in create statement", c.Name)
	}
	expr := scanExpr(s.S[idx[0][1]-1:])
	if expr == "" {
		return fmt.Errorf("spanner: unexpected empty generation expression for column %q", c.Name)
	}
	typ := virtual
	if f == 3 {
		typ = stored
...
const (
	...
	tablesQuery = "SELECT `name`, `sql` FROM spanner_master WHERE `type` = 'table' AND `name` NOT LIKE 'spanner_%'"
	...
	indexesQuery = "SELECT `il`.`name`, `il`.`unique`, `il`.`origin`, `il`.`partial`, `m`.`sql` FROM pragma_index_list('%s') AS il JOIN spanner_master AS m ON il.name = m.name"
