	...
	for _, tbl := range tbls {
		for _, col := range tbl.Columns {
			if col.Type.IsRef {
				...
				if !ok {
					return fmt.Errorf("cockroach: table %q not found in schema %q", tbl.Name, sch.Name)
				}
...
func resolveEnum(ref *schemaspec.Type, enums []*Enum) (*Enum, error) {
	...
	return nil, fmt.Errorf("cockroach: enum %q not found", n)
}
...
func enumName(ref *schemaspec.Type) (string, error) {
	...
	if len(s) != 2 {
		return "", fmt.Errorf("cockroach: failed to extract enum name from %q", ref.T)
	}
	return s[1], nil
}
...
func formatTime() schemahcl.TypeSpecOption {
	return schemahcl.WithTypeFormatter(func(t *schemaspec.Type) (string, error) {
		...
		p, err := a.Int()
		if err != nil {
			return "", fmt.Errorf(`cockroach: parsing attribute "precision": %w`, err)
		}
