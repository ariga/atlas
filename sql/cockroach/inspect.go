...
func (i *inspect) InspectSchema(ctx context.Context, name string, opts *schema.InspectOptions) (s *schema.Schema, err error) {
	...
	case n == 0:
		return nil, &schema.NotExistError{Err: fmt.Errorf("cockroach: schema %q was not found", name)}
	case n > 1:
		return nil, fmt.Errorf("cockroach: %d schemas were found for %q", n, name)
	}
	r := schema.NewRealm(schemas...).SetCollation(i.collate)
	r.Attrs = append(r.Attrs, &CType{V: i.ctype})
...
func (i *inspect) columns(ctx context.Context, s *schema.Schema) error {
	...
	if err != nil {
		return fmt.Errorf("cockroach: querying schema %q columns: %w", s.Name, err)
	}
	defer rows.Close()
	for rows.Next() {
...
func (i *inspect) enumValues(ctx context.Context, s *schema.Schema) error {
	...
	if err != nil {
		return fmt.Errorf("cockroach: querying enum values: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var (
		...
		if err := rows.Scan(&id, &v); err != nil {
			return fmt.Errorf("cockroach: scanning enum label: %w", err)
		}
...
func (i *inspect) indexes(ctx context.Context, s *schema.Schema) error {
	...
	if err != nil {
		return fmt.Errorf("cockroach: querying schema %q indexes: %w", s.Name, err)
	}
	defer rows.Close()
...
func (i *inspect) partitions(s *schema.Schema) error {
	for _, t := range s.Tables {
		...
		default:
			return fmt.Errorf("cockroach: unexpected partition strategy %q", s)
		}
		idxs := strings.Split(strings.TrimSpace(d.attrs), " ")
		if len(idxs) == 0 {
			return fmt.Errorf("cockroach: no columns/expressions were found in partition key for column %q", t.Name)
		}
		for i := range idxs {
			switch idx, err := strconv.Atoi(idxs[i]); {
			case err != nil:
				return fmt.Errorf("cockroach: faild parsing partition key index %q", idxs[i])
			// An expression.
			case idx == 0:
				j := sqlx.ExprLastIndex(d.exprs)
				if j == -1 {
					return fmt.Errorf("cockroach: no expression found in partition key: %q", d.exprs)
				}
			...
			default:
				if idx > len(t.Columns) {
					return fmt.Errorf("cockroach: unexpected column index %d", idx)
				}
				d.Parts = append(d.Parts, &PartitionPart{
					C: t.Columns[idx-1],
				})
...
func (i *inspect) fks(ctx context.Context, s *schema.Schema) error {
	...
	if err != nil {
		return fmt.Errorf("cockroach: querying schema %q foreign keys: %w", s.Name, err)
	}
	defer rows.Close()
...
func (i *inspect) checks(ctx context.Context, s *schema.Schema) error {
	...
	if err != nil {
		return fmt.Errorf("cockroach: querying schema %q check constraints: %w", s.Name, err)
	}
	defer rows.Close()
...
func (i *inspect) addChecks(s *schema.Schema, rows *sql.Rows) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&table, &name, &clause, &column, &indexes, &noInherit); err != nil {
			return fmt.Errorf("cockroach: scanning check: %w", err)
		}
		t, ok := s.Table(table)
		if !ok {
			return fmt.Errorf("table %q was not found in schema", table)
		...
		if _, ok := t.Column(column); !ok {
			return fmt.Errorf("cockroach: column %q was not found for check %q", column, name)
		}
		check, ok := names[name]
		if !ok {
...
func (i *inspect) schemas(ctx context.Context, opts *schema.InspectRealmOption) ([]*schema.Schema, error) {
	...
	rows, err := i.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("cockroach: querying schemas: %w", err)
	}
	defer rows.Close()
...
FROM
	INFORMATION_SCHEMA.TABLES AS t1
	JOIN pg_catalog.pg_namespace AS ns ON ns.nspname = t1.table_schema
	JOIN pg_catalog.pg_class AS t2 ON t2.oid = ns.oid
