...
func FormatType(t schema.Type) (string, error) {
	...
	case *schema.EnumType:
		if t.T == "" {
			return "", errors.New("cockroach: missing enum type name")
		}
		f = t.T
	...
	case *schema.StringType:
		...
		default:
			return "", fmt.Errorf("cockroach: unexpected string type: %q", t.T)
		}
	...
	case *schema.DecimalType:
		...
		case TypeDecimal:
			f = TypeNumeric
		default:
			return "", fmt.Errorf("cockroach: unexpected decimal type: %q", t.T)
		}
		switch p, s := t.Precision, t.Scale; {
		case p == 0 && s == 0:
		case s < 0:
			return "", fmt.Errorf("cockroach: decimal type must have scale >= 0: %d", s)
		case p == 0 && s > 0:
			return "", fmt.Errorf("cockroach: decimal type must have precision between 1 and 1000: %d", p)
		case s == 0:
	...
	case *SerialType:
		...
		case TypeSerial8:
			f = TypeBigSerial
		default:
			return "", fmt.Errorf("cockroach: unexpected serial type: %q", t.T)
		}
	...
	case *schema.UnsupportedType:
		return "", fmt.Errorf("cockroach: unsupported type: %q", t.T)
	default:
		return "", fmt.Errorf("cockroach: invalid schema type: %T", t)
	}
	return f, nil
}
...
func parseColumn(s string) (*columnDesc, error) {
	...
	case TypeDecimal, TypeNumeric:
		if len(parts) > 1 {
			...
			if err != nil {
				return nil, fmt.Errorf("cockroach: parse precision %q: %w", parts[1], err)
			}
		}
		if len(parts) > 2 {
			c.scale, err = strconv.ParseInt(parts[2], 10, 64)
			if err != nil {
				return nil, fmt.Errorf("cockroach: parse scale %q: %w", parts[1], err)
			}
		}
	case TypeBit:
	...
	case TypeTime, TypeTimeTZ, TypeTimestamp, TypeTimestampTZ:
		...
		if len(parts) > 1 && reDigits.MatchString(parts[1]) {
			...
			if err != nil {
				return nil, fmt.Errorf("cockroach: parse time precision %q: %w", parts[1], err)
			}
			p = i
...
func parseCharParts(parts []string, c *columnDesc) error {
	...
	if err != nil {
		return fmt.Errorf("cockroach: parse size %q: %w", parts[1], err)
	}
	c.size = size
	return nil
}
