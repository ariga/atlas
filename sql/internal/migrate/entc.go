...
const (
	initTpl = `// Client is the client that holds all ent builders.
type Client struct {
	config
	{{- range $n := $.Nodes }}
...
func (c *Client) init() {
	...
	testTpl = `{{ with extend $ "Package" "enttest" -}}
	{{ template "header" . }}
{{ end }}
...
`,
		...
		Templates: []*gen.Template{
			gen.MustParse(gen.NewTemplate("client/init").Parse(initTpl)),
			gen.MustParse(gen.NewTemplate("enttest").Parse(testTpl)),
		},
	})
	if err != nil {
