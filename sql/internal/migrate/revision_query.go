...
func (rq *RevisionQuery) prepareQuery(ctx context.Context) error {
	for _, f := range rq.fields {
		if !revision.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("migrate: invalid field %q for query", f)}
		}
	}
	if rq.path != nil {
...
func (rq *RevisionQuery) sqlExist(ctx context.Context) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("migrate: check existence: %w", err)
	}
	return n > 0, nil
}
