...
func (*diff) ForeignKeyAttrChanged(from, to []schema.Attr) bool {
	if foreignKeyDeferrable(from) != foreignKeyDeferrable(to) {
		return true
	}

	return foreignKeyDeferred(from) != foreignKeyDeferred(to)
}
...
// foreignKeyDeferrable returns the DEFERRABLE value from the foreign key attributes.
func foreignKeyDeferrable(attrs []schema.Attr) bool {
	if i := (Deferrable{}); sqlx.Has(attrs, &i) {
		return i.V
	}
...
// foreignKeyDeferrable returns the INITIALLY DEFERRED value from the foreign key attributes.
func foreignKeyDeferred(attrs []schema.Attr) bool {
	if i := (Deferrable{}); sqlx.Has(attrs, &i) {
		return i.Default
	}
	// The default PostgreSQL behavior.
