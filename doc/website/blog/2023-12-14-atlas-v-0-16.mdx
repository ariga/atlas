...
* **Review Policies** - Users working with declarative migrations can now define "review policies" which
  can define thresholds for which kinds of changes require a human review and which can be auto-applied.
...
ClickHouse is a high-performance, columnar database optimized for analytics and real-time query processing.
...
of changes require a human review and which can be auto-applied.
...
are above a certain risk threshold. For example, you can configure Atlas to only ask for review if any warning are found
...
A few other notable features shipping in this release are:
