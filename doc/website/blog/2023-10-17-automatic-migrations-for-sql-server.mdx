...
To skip the intro and go straight to the demo, click [here](#demo).
...
```
Atlas will connect to our target database to inspect it's current state, then it will use the dev-database to
normalize our schema and then generate the SQL commands that will bring our database to the desired state:
```sql
-- Planned Changes:
-- Create "users" table
...
```
Atlas prompts use to approve the changes before applying them to the database:
```
...
Atlas will prompt you to ask whether you would like to create your visualization publicly (in a publicly
