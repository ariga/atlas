...
As applications today are expected to be highly-available, which means that we cannot
shut them down for maintenance, during the deployment phase of a new version we will
always have old and new code serving traffic side by side. This means, for example, that
renaming a column from `email_address` to `email` will cause errors applications running
the previous version of the schema reference the old column name in their queries.
