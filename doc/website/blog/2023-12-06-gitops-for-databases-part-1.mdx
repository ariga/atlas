...
In this tutorial, we show to apply the fourth principle to your database migrations using the Atlas Operator and ArgoCD.
...
4. The GitHub CLI, `gh`. To install `gh`:
   ...
   brew install gh
   ```
  For instructions in other platforms, follow the instructions [here](https://github.com/cli/cli#installation).
...
In this section, we will set up a GitHub Actions workflow that will run the Atlas to your CI/CD
pipeline.
...
Now that we have set everythign up, let's test our pipeline, end-to-end. To do so, we will
...
Based on the configuration in the GitHub Action workflow we created, Atlas will automatically
...
case the "users" table is not empty. The second is that creating an non-concurrently causes write locks on the "users" table.
