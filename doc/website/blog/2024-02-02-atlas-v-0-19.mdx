...
risky migrations.
* [_Schema Docs__](#schema-docs): Atlas lets you manage your database schema as code. One of the things we love most
...
The code below presents a simple example of pre-migration checks. The default checks file is named `checks.sql`,
...
In a [recent version](/blog/2024/01/01/atlas-v-0-17), we released trigger support for PostgreSQL, MySQL and
...
5. Now, let's define the logic to assign a `grade_status` using a `TRIGGER`. Append
   ...
   ```hcl title=schema.hcl (continued)
    trigger "after_grade_insert" {
      ...
      after {
        insert = true
        update_of = [table.grades.column.grade]
      }
      as = <<-SQL
       BEGIN
         SET NOCOUNT ON;
...
6. Finally, let's test our application to see that it actually works. We can do this by populating our database with
   some students' grades. To do so, connect to the SQL Server container and open a `sqlcmd` session. Update your container
   name or ID when copying the command below:


    ```bash
    docker exec -it <container_name_or_id> /opt/mssql-tools/bin/sqlcmd -S localhost -U SA -P 'P@ssw0rd0995'
    ```
   ...
   ```bash
         docker exec -it <container_name_or_id> /opt/mssql-tools/bin/sqlcmd -S localhost -U SA -P 'P@ssw0rd0995' -Q "SELECT * FROM grades;"
   ```
