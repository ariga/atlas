...
Trigger support is available for logged-in users only. To use this feature, run:
...
code when certain events occur on a table. For example, you can use triggers to
automatically update the amount of stock in your inventory when a new order is
placed, to create an audit log of changes to a table. Using this event-based approach,
...
### Improved ERDs
A few releases ago, we announced [official support](/atlas-v-0-13) for database views. "Views" are virtual tables created
by a query and stored in the database. Now, you can visualize these database objects in the ERD.

![erd-views](https://atlasgo.io/uploads/images/erd-views2.gif)
...
![erd-filters](https://atlasgo.io/uploads/images/erd-filters.png)
