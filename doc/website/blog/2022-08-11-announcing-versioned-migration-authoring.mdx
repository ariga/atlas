...
Start by downloading the Atlas CLI. The following command will work for MacOS:
```bash
brew install ariga/tap/atlas
```
For instructions on other platforms [go here](cli/getting-started/setting-up#install-the-cli).

Start by defining a simple Atlas schema with one table and an empty migration directory:
...
Observe two files were created in the `migrations` directory:
