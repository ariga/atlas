...
We believe, that in an ideal world, schema migrations on production grade databases are done in an automated way,
...
Atlas now can periodically check if your deployed databases schemas match their desired state. Among others, this
features requires [Atlas Cloud](https://atlasgo.cloud/) to connect to your database. As it is uncommon for databases
