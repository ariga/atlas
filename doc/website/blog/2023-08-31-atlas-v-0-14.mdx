...
of them from start to finish to set up your database. Depending on your setup this  can take a few seconds or more.
...
Observe that as expected, Atlas skipped all of the migrations up to the checkpoint and only applied the last one!
...
There are multiple ways to accomplish this, such as building custom Docker images, as most methods covered in the
...
the cloud, just as you would execute `docker run` to run a container image that is stored in Docker container registry.
...
4. Edit away! (and enjoy syntax highlighting, code completion and warnings)
