...
Atlas can help you recover quickly from such failures and quickly get back on track.
...
you execute fails. By utilizing these features, your team can greatly reduce MTTR for
...
3. Data-dependent changes - If migrations contain the manipulation of data or the modification
  of constraints, the operation may fail, depending on existing data in the target database.
...
troubleshoot and resolve migration failures. We will discuss:
...
The first step to solving any failure is being to triage the issue at hand. To assist
...
which meaning they cannot provide any assistance in cases of partial failures.

Atlas, on the other hand, actually parses the migration files prior to executing
...
This is done, in theory, to allow users to seamlessly return to a previous version
...
* `exclude` - tells Atlas to ignore `atlas_schema_revision` which is a metadata
