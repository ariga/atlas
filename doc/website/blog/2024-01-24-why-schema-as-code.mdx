...
in plain SQL or in [HCL](/quides/ddl#hcl). The schema is saved in a schema file, which holds the _desired_ state of our
database.
...
Now let's run the `migrate apply` command again, and view the migration plan that Atlas will create for us:
