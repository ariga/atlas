...
database upgrade scripts (usually in SQL), called "migrations", that would be executed by the migration tool.
This trend was further intensified with the rise of DevOps and CI/CD, as it became the de-facto standard way for
automating the management of database schemas.
...
infrastructure. It allows for better collaboration between team members, it is more robust and less error-prone,
it is self documenting, it allows for better visibility, governance and control into the state of the infrastructure.
...
for making sure that the database schema is consistent and that all changes are done in a safe way.

In recent years, as cloud-native, microservices-based architectures have become more popular, the amount of databases
that each team manages has grown significantly. This has led to a situation where many teams don't have any DBAs
on their team, and the responsibility for managing the database schema falls on the shoulders of the developers.
...
This process is time-consuming and error-prone. Even worse, it turns certain people with more database
...
and other safety mechanisms that help you catch errors and issues before they become a full-blown production outage.
