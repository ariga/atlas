...
Despite their numerous benefits, views are often underutilized in many applications. Softwares like ORMs play a key role 
in educating developers and increasing the adoption of views. However, many ORMs only provide partial support for views.

Let's explore how GORM, one of the most popular ORMs in Go, enables users to create and utilize views in 
their applications. 
...
Next, to represent our view, we need to define a corresponding struct:
...
Finally, apply a query to select the desired records from the view:
...
Interacting with database views in GORM can initially feel a bit tricky. Here's why:
* GORM is centered around structs, which serve as the foundation for structuring and querying data. 
Structs are the source of truth for the data - this is the core principle of ORMs.
* However, the GORM API for views doesn't quite align with this principle, leading to a
 slightly cumbersome workflow. We wish migrations, querying, everything would work from the same struct definition. 
...
and capabilities) came up with a neat solution. Here's what we did:
