...
GORM is a fantastic ORM library that supports auto creation for database tables 
with the [AutoMigrate](https://gorm.io/docs/migration.html#Auto-Migration) feature. Regarding views, however, GORM does not provide 
a developer-friendly way to [manage them](https://gorm.io/docs/migration.html#Views).
...
We've seen many developers complain about this. Since views are just virtual tables, they should be managed in the same way as tables.

GORM migration does its job, but it's only sufficient for simple cases. When 
you need to fully control and adapt [versioned migrations](/concepts/declarative-vs-versioned#versioned-migrations) but 
