...
When this model evolves, the database's schema must change as well. No one wants (or should)
to connect to their production database and apply changes manually, which is why
...
is to present 3 popular choices for migration tools for Go projects to help you in this choice.
...
workflows migration linting, and a GitHub Actions integration.


### Golang migrate
Golang migrate was initially created by [Matt Kadenbach](https://github.com/mattes), in 2018 the project was [handed
over](https://github.com/mattes/migrate/issues/311) to [Dale Hui](https://github.com/dhui), today the project resides
...
[Pressly](https://github.com/pressly) created a fork in 2016 improving the usage, adding support for
...
an entire infrastructure of _static code analysis_ was built that is able to provide warnings
about dangerous or inefficient operations, this technique is called **migration linting** and can be
integrated with [Atlas Github action](https://atlasgo.io/integrations/github-actions) during CI.
...
Another key point that Atlas solves is handling migrations integrity, this is a huge problem when working with
