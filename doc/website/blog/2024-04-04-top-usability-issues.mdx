...
After many years of building software professionally, we've come to realize that the most stressful, tedious and error-prone part of
...
One of the most important principles that came from the DevOps movement is that idea that to achieve effective
...
and we better have some way to ensure it is compatible with the code we are deploying.
...
I need to run them in order to understand the current state of the database.
...
and error-prone, especially if you have developers who don't have a lot of experience with operating databases.

Classic migration tools were created a time when tech-stacks were much simpler and less diverse. In addition,
...
can be a daunting and stressful task.  Even if it is something you are good at, it's still something that requires
...
some require consideration and research.
...
undetected issues in production when they are deployed. To overcome this, team develop their own processes to
...
Suppose you planning a migration to version `N+1` of the database, from version `N1`.  The migration script for version
