...
* **Built-in schema visualization** - Over the past year we released some tools to help developers visualize their database
  schemas using Entity Relationship Diagrams (ERDs). Starting today, you can use the `atlas schema inspect --visualize`
...
last few months, we have been working on a driver for MS-SQL with some of our enterprise design partners and are happy today
...
```shell
atlas schema apply --url="sqlserver://sa:P@ssw0rd0995@localhost:1433?database=master" -f schema.hcl
```
...
```shell
atlas schema inspect --url="sqlserver://sa:P@ssw0rd0995@localhost:1433?database=master"
```
