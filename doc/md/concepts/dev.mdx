...
Atlas does not support yet initializing a ClickHouse dev-database through `docker://`, but you can create a local container by running the following command:

```shell
docker run -d --rm --name clickhouse-dev -p 9000:9000 -d clickhouse/clickhouse-server:latest
```
