...
Redshift is a managed service and does not support running in a Docker container. To user dev-database with Redshift,
we recommend using a real Redshift cluster with separate database.
...
The `docker` block is available for [Atlas Pro users](/features#pro-plan). To use this feature, run:
