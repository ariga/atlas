...
Connecting to a specific Redshift cluster with schema named `public`:
```shell
redshift://user:pass@redshift-cluster:5439/database?search_path=public
```

Connecting to a specific Redshift cluster with schema named `public` with SSL disabled:
```shell
