...
* `--dev-url` - a [URL](/concepts/url) to the [_Dev-Database_](../concepts/dev.md).
...
* `--log` (optional) - [Go template](https://pkg.go.dev/text/template) to use to format the output.
...
Atlas with a connection string to a [_Dev-Database_](../concepts/dev.md).
...
The following example demonstrates how to use [Atlas DDL](../atlas-schema/sql-resources) (HCL) as the desired state and
...
1\. If a migration file(s) was already deleted, you can use the [command above](#migration-directory) to migrate the
database state to the state defined by the migration directory. For example:
...
2\. If the file is still present, the `version` parameter can be used to specify that the database should be migrated
to the state defined in a specific version of the migration directory. For example:
...
3\. In order to clean the entire schema, use the `schema clean` command, and then use
[`atlas migrate apply`](../versioned/apply) to replay the migration files. For example:
