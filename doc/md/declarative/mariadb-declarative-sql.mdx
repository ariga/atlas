---
id: mariadb-declarative-sql
title: Getting started with SQL declarative migrations for a MariaDB schema
slug: /declarative/mariadb-declarative-sql
---
...
:::info
In this article, we will be focusing on SQL-based schema inspection with Atlas. For those opting to use an HCL schema, read the documentation [here](https://atlasgo.io/declarative/mariadb-declarative-hcl).
:::
...
CREATE TABLE `orders` (`id` int NOT NULL AUTO_INCREMENT, `customer_id` int NOT NULL, `order_date` date NOT NULL, PRIMARY KEY (`id`), INDEX `customer_id_fk` (`customer_id`), CONSTRAINT `customer_id_fk` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
...
CREATE TABLE `orders` (
    ...
    CONSTRAINT `customer_id_fk` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`) ON UPDATE RESTRICT ON DELETE RESTRICT
) CHARSET utf8mb4 COLLATE utf8mb4_general_ci;
```
