...
By default, `atlas schema plan` propose pushing the plan file to the Atlas registry. However, you can use the `--save` flag
...
By default, `atlas schema plan` pushes **approved** plans to the registry. However, in some cases, we prefer to create
...
2. Automated (CI) workflow:
   1. When setting the `schema/plan` GitHub Action for your repository, Atlas automatically creates a plan in pending state.
