...
:::info Login to Atlas
By default, running `atlas schema diff` diffs only **schemas**, **tables**, and their associated **indexes** and
...
and additional features are available to logged-in users only. To include these resources in the schema diffing, use the
following command:
...
Compare two Redshift clusters with schema named `public` under the
`example` database:
