...
```shell
atlas schema diff \
  --from "clickhouse://localhost:9000/default" \
  --to "clickhouse://localhost:9000/dev"
```
...
```shell
atlas schema diff \
  --from "clickhouse://user:pass@localhost:3306/example" \
  --to "clickhouse://user:pass@remote:3306/example"
```
...
atlas schema diff \
  --from "file://migrations" \
  --to "clickhouse://user:pass@remote:9000/test" \
  --dev-url "clickhouse://localhost:9000"
```
