...
title: Getting started with HCL declarative migrations with a MySQL schema
...
Once you're connected, you can add a table in it by running the following command:
...
```
:::info
To learn more about inspecting schema with Atlas in variety of use cases, visit the official documentation [here](https://atlasgo.io/declarative/inspect)
:::

## Step 2: Planning a change

Once you have inspected your database, the next step is to plan a change. This is done by modifying the HCL file in a way that it defines the desired state of your schema. The file should include the changes you want to make to your database, such as adding tables, columns, or indexes.

:::info
In this article, we will be focusing on HCL-based schema inspection with Atlas. For those opting to use SQL schema, please refer to the official documentation [here](https://atlasgo.io/declarative/declarative-sql).
:::
...
The final step is to apply the change to your database. To do this, simply run the `atlas schema apply` command with the required flags. This command will compare the current state of your database to the desired state defined in your HCL file and make the necessary changes.
...
:::info
Atlas supports both Declarative and Versioned workflows for managing database schemas. The choice between the two depends on your requirements. To understand the difference and choose the best option for you, visit the official documentation on Declarative vs Versioned workflow [here](https://atlasgo.io/concepts/declarative-vs-versioned).
:::
