...
```shell
kubectl create secret generic atlas-agent --from-literal api-key=YOUR_API_KEY
```
...
```shell
kubectl create secret generic atlas-agent-db --from-literal password=YOUR_DB_PASSWORD
```
...
```shell
helm install atlas-agent oci://ghcr.io/ariga/charts/atlas-agent
```
