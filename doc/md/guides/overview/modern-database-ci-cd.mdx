...
CI/CD is not just for application code.  Teams can also apply CI/CD principles to their databases,
to extend the benefits of CI/CD to one of the most critical components of their stack.

This document describes the recommended workflow for teams using Atlas to achieve continuous integration
and continuous delivery (CI/CD) for their databases.
...
2. **Changes to the database schema are stored in a versioned migration directory.** All planned changes
   ...
   which are executed in order to apply the changes to the database.
...
Create a new branch to track this change to the database.
...
atlas migrate lint --base atlas://project-id --context $ATLAS_CONTEXT
```
...
It is possible to push your changes to Atlas Cloud directly with the CLI
...
atlas migrate push project-id:$GIT_COMMIT
atlas migrate push project-id:latest
```
...
atlas migrate apply --url mysql://user:pass@host:port/dbname --dir atlas://project-id:latest
```
