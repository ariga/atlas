...
slug: /guides/clickhouse/getting-started
tags: [clickhouse]
---
...
you’re probably not going to feel much of this pain. But for mission-critical applications complex and interconnected
schemas moving fast without breaking things becomes difficult.
...
Let's start off with a minimal database with a `users` table and an `id` as the primary key.
...
columns. The `schema` field references the `demo` schema that is defined elsewhere
in this document. In addition, the `primary_key` sub-block defines the `id` column as
the primary key for the table. Atlas strives to mimic the syntax of the database that the user is working against. In
this case, the type for the `id` column is `UInt64`, and `String` for the `name` column.
...
The declarative approach requires the user to define the desired end schema, and Atlas provides a safe way to
alter the database to get there. Let's see this in action.
...
The migration file represents the current state of our database, and the sum file is in charge of maintaining the integrity
...
Run `ls migrations`, and you'll notice that a new migration has been generated. Let's run `atlas migrate push` again and
...
Next, we recommend you [set up CI](/cloud/setup-ci) for your migration directory, to ensure that all changes applied to
your database are safe and automatically updated when pushed.

For more in-depth guides, check out the other pages in this section or visit our [Docs](/getting-started) section.
