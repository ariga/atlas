...
manage their database schemas using its [schema generation](https://docs.jboss.org/hibernate/orm/current/userguide/html_single/Hibernate_User_Guide.html#schema-generation) feature,
...
If you don't have a Hibernate project handy, check out [hibernate getting started page](https://docs.jboss.org/hibernate/orm/current/userguide/html_single/Hibernate_User_Guide.html)
...
Verify that the plugin is installed, run this command
...
### Library vs Gradle task

The Atlas Hibernate Provider can be used in two modes:
* **Gradle Task** - In most cases, you can use the provider as an gradle task to output
the Hibernate schema and load the schema into Atlas.
* **Library** - For other case, if you are initializing Hibernate in a custom way,
using an older version of Hibernate or if you are not using gradle,
you can use the provider as a library and create a small application that prints the schema to stdout.
...
And depending on your database, add an environment configuration. For example, for MySQL and PostgreSQL:
...
properties file and in the dev database that Atlas will use.

Create a resource named `schema-export.propertie` in `src/main/resource`, it should be part of the classpath by default.
...
`ConsoleSchemaManagementTool`. In this example, we are going to use Spring for to initialize Hibernate.

> By default, the `HibernateSchemaManagementTool` will try to connect to the
database during initialization and generate the schema into the database.
...
And lastly, in your project directory, create a new file named `atlas.hcl` with the following contents
...
Using the configuration we created earlier, we can generate a migration file by running this command:
