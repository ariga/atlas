...
to manage both Postgres enums and the GORM schema as a single migration unit using Atlas.
...
2\. In your GORM schema, define an enum field that uses the underlying Postgres `ENUM` type:
...
CREATE TYPE "status" AS ENUM ('active', 'inactive', 'pending');
-- Create "users" table
CREATE TABLE "users" ("id" bigserial NOT NULL, "created_at" timestamptz NULL, "updated_at" timestamptz NULL, "deleted_at" timestamptz NULL, PRIMARY KEY ("id"));
