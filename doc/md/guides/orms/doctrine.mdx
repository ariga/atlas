...
Doctrine is a popular ORM widely used in the PHP community, used by projects like Symfony. 
...
However, at some point, teams need more control and decide to employ [versioned migrations](/concepts/declarative-vs-versioned#versioned-migrations) methodology.
...
More of the limitations of Doctrine migration tool include:

* **Ensuring Migration Safety.** Migrations are a risky business. If you're not careful, you can easily cause data loss or 
  a production outage. Doctrine migration tool does not provide a native way to ensure that a migration is safe to apply.
* **Modern Deployments.**  Doctrine does not provide native integration with modern deployment practices such as [GitOps](/blog/2023/12/06/gitops-for-databases-part-1)
...
Atlas works in similar way to Doctrine migration tool, by calculating the diff between the _current_ state of the database,
...
which is a small program that can be used to load the schema of a Doctrine project into Atlas.

In this guide, we will show how Atlas can be used to automatically plan schema migrations for Doctrine users.

## Prerequisites

* A local [Doctrine](https://www.doctrine-project.org/projects/doctrine-orm/en/3.1/index.html) project.

If you don't have a Doctrine project, you can create one by following the [Getting Started](https://www.doctrine-project.org/projects/doctrine-orm/en/3.1/tutorials/getting-started.html) guide.
...
<Tabs>
<TabItem value="v2" label="Dcotrine 2" default>
...
<TabItem value="v3" label="Doctrine 3">
...
### Doctrine Command vs Script mode

The Atlas Doctrine Provider can be used in two modes:

* **Doctrine Command** - If all of your Doctrine entities exist under single directory,
...
Next, alter the `User` entity to include `$name` property.
