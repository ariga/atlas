...
1. **The SQL is not transparently visible in merge/pull requests for review purposes**: 
Migration files generated by EF Core do follow the _versioned migrations_ pattern, but each 
file contains the operations necessary to apply the migration (in `Up`) and to revert it (in `Down`), instead 
of the SQL statements that would be executed in a versioned migration.
  ...
  protected override void Up(MigrationBuilder migrationBuilder)
  {
    migrationBuilder.CreateTable(
      ...
      columns: table => new
      {
        ...
        ConcurrencyStamp1 = table.Column<string>(type: "TEXT", nullable: true)
      },
      constraints: table =>
      {
...
2. **The designer files repeat the entire schema for each migration**: Along with migration files, EF Core also
 generates a designer file for each migration. This file contains the entire schema for the migration, with the 
 intention of maintaining the immutability of each migration. However, these files tend to be very large and grow 
 linearly with the number of migrations, making them difficult to review and manage.
