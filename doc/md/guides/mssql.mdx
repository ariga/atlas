...
[Microsoft SQL Server](https://www.microsoft.com/en-us/sql-server/) (MSSQL) is a powerful relational database management
system that has one of the prominent enterprise-grade data solutions for decades. Commonly used by enterprises, MSSQL
can efficiently handle growing amounts of data and users, making it easy to scale.

However, managing a large database schema in MSSQL can be challenging due to the complexity of related data
...
In this guide, we will dive into setting up Atlas for MSSQL, and introduce the different workflows available.
...
To use MSSQL with Atlas, you'll need to [log in to Atlas](https://auth.atlasgo.cloud/signup). If it's your first time,
...
To create this on our MSSQL database, run the following command:
...
To inspect our locally-running MSSQL instance, use the `-u` flag and write the output to a file named `schema.hcl`:
...
To inspect our locally-running MSSQL instance, use the `-u` flag and write the output to a file named `schema.sql`:
...
Next steps:
* Read the [full docs](/atlas-schema/hcl) to learn HCL schema syntax or about specific MSSQL [column types](/atlas-schema/hcl-types#sql-server)
