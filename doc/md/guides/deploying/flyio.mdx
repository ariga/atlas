...
the deployment of the app in different regions and routing requests based on the user latency.
...
In this guide, we will demonstrate how Atlas can be integrated with the Fly.io deployment process by using a Dockerfile.
We will assume that you have already have a Fly project and are able to deploy it.

## Release command

Fly supports running a command during a new release. The command is only executed during the release phase and
...
Using Docker multi-stage builds, we can create an easy-to-use Dockerfile. In this guide we will use a Go app as an example.
...
If you are using a different programming language, most of the time you will only have to change the app stage.
...
a [behavior on Fly's side](https://community.fly.io/t/using-env-variables-in-release-command/7186),
...
insights on the database schema and migrations, we recommend giving [Ariga Cloud](https://atlasgo.io/cloud/getting-started) a try.