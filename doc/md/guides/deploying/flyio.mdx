...
converts Docker images (OCI images) to [Firecracker microVMs](https://firecracker-microvm.github.io/), allowing
the deployment of the app in different regions and routing the requests based on the user latency.

Apps on Fly can be [deployed in one of three ways](https://fly.io/docs/reference/builders/), using a Dockerfile, a
Docker image or a buildpack.

In this guide, we will demonstrate how Atlas can be integrated with Fly.io deploy process using a Dockerfile.
We will assume that you have a project already created on Fly and are able to deploy it.

## Release command

Fly has support for running a command during a new release, the command is executed only during the release phase and
if it exits with a status other than zero, Fly marks the deployment as failed. The release command is the
[recommended way](https://fly.io/docs/reference/configuration/#the-deploy-section) to run database migrations on Fly.

:::note
The release command only allows executing commands that are present in the application image, that means that we
have to embed the Atlas binary with our application, [we usually suggest separated step for handling the
migration](https://atlasgo.io/guides/deploying/intro#running-migrations-as-part-of-deployment-pipelines),
but since this is a requirement from Fly, we don't have an option.
:::

## Defining the Dockerfile

Using Docker multi-stage builds, we can create an easy-to-use Dockerfile, in this guide we will use a Go app as example,
...
If you are using a different programming language, most of the time you will have to only change the app stage.
If your application requires a runtime you may have to change the final stage as well.

:::info
It's important for the final image to have bash on it, this is mostly
a [behavior on Fly side](https://community.fly.io/t/using-env-variables-in-release-command/7186),
where expanding environment variables don't seem to work correctly on the release command.
:::
...
defining [secrets](https://fly.io/docs/reference/secrets/), sensible values that are available during runtime as
...
To tell Fly to execute the release command during a deployment, we only have to add a `deploy` block with the release
command provided:

```toml title="fly.toml"
[deploy]
release_command = "bash -c '/atlas migrate apply --url $DATABASE_URL'"
```
...
and [Fly](https://fly.io/docs/app-guides/continuous-deployment-with-github-actions/) Github action, for even more
insight of the migrations and database schema we recommend giving a try to [Ariga Cloud](https://atlasgo.io/cloud/getting-started).