...
One of the common things IaC pipelines need to achieve when preparing the infrastructure for
the deployment of applications is to ensure the required schemas (named databases) exist on the
...
which allows teams to provision database resources as part of thei IaC pipelines. Let's see how.
...
That is, despite us providing a specific definition of some aspect of the schema, the database
will store it in another, equivalent form. This means in certain situations it may appear to
...
The `src` attributes defines the desired schema we wish to apply to our
