---
id: testing-domains
title: Testing Database Domains
...
In this guide we will learn how to use Atlas's [`schema test`](/testing/schema) command to test database domains.


## Database Views
A `domain` is a user-defined data type that is based on an existing  data type but with optional constraints and d
efault values. Supported by PostgreSQL.
...
```hcl title="schema.hcl"
test "schema" "us_postal_code_validation_valid" {
    parallel = true
    for_each = [
...
test "schema" "us_postal_code_validation_invalid" {
    parallel = true
    for_each = [
    ...
    exec {
        sql = <<-SQL
        SELECT '${each.value.input}'::us_postal_code
        WHERE '${each.value.expected}' = 'valid'
        SQL
    }
...
we use the [`catch`](/testing/schema#catch-command) command, so that the invalid inputs don't cause
our test case to fail.
