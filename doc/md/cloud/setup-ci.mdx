...
* **Pull (GitHub App)** - You install the Atlas Cloud GitHub App on your GitHub
  repository and Atlas Cloud will automatically pull changes from your GitHub
  repository and run tests on them.
...
* **Push (GitHub Action)** - You install the [`ariga/atlas-action`](https://github.com/ariga/atlas-action)
  ...
  This is a bit more work to set up, but it doesn't require you to grant Atlas
...
Select the repositories you want to grant access to and click "Install and Authorize".
...
1. Make sure you have the necessary permissions to configure your action:

  ```shell
  gh auth refresh -s write:packages,workflow
  ```

2. Make sure your current working directory is the root of your repository.

  ```shell
  cd path/to/my/repo
  ```

3. Run the `init-ci` command to configure the GitHub Action:
  ...
  Atlas will then which database driver this directory contains migrations for. Select the
