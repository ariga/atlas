...
The Agent is running now and can connect to Atlas. However, in order to connect to your databases, it needs to know
...
authentication, since in this case you need to have access to the secrets store, usually using IAM authentication.
Find the secret name and region for your secret. If you need more info,
[read this](/guides/deploying/secrets##using-iam-authentication). If your secret is stored as JSON, e.g. when you chose
to let RDS handle your database password in Screts Manager , you can provide the path to the actual token using a
dot-notation, e.g. for a secret like `{"password":"my_passw0rd!"}` you'd provide `.password` as path to the token.
...
GCP Secrets Manager is a secret store you can use to retrieve a secret from. Find the project id and secret name od your
secret. If you need more info, [read this](/guides/deploying/secrets##using-iam-authentication). If your secret is
stored as JSON, you can provide the path to the actual token using a dot-notation, e.g. for a secret like
`{"password":"my_passw0rd!"}` you'd provide `.password` as path to the token.

</TabItem>
<TabItem value="env" label="Environment Variable">

You can tell the Agent to look for the password in a environment variable. E.g. if your password lives in an environment
variable like `DATABASE_PASSWORD=passw0rd` you'd provide `DATABASE_PASSWORD` to the agent, not the actual.
...
information about the diff., including an ERD, HCL diff and SQL statements required to fix the drift.

:::danger
Do not apply the SQL blindfolded to fix the drift. It is potentially destructive.
:::
...
You can instruct Atlas to notify you if there is a drift. Atlas support various channels, e.g. via email, Slack,
