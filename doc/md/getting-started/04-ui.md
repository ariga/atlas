...
When the UI opens, we are taken to the schema page. This screen displays an overall look of our Atlas schema.
...
Now that we have visualized our schema using the UI, let’s see how to initiate a migration.
...
Going back to our schema page, we can notice a few changes. Firstly, our schema is now out of sync. When a schema is out of sync, two separate schemas appear: desired and inspected.
...
Looking at the ERD, we can see that our table `likes` is green, showing that it has been added. This means the change has been recorded, but not yet applied.
