...
and avoiding stepping on each other's toes. Atlas also analyzes new schema changes in the migration directory that might
...
Teams that connected their project to Atlas Cloud (see [setup](/cloud/setup-ci) for more details), can simply run
...
By using this method, Atlas also warns you if your local migration directory is not up-to-date with the latest state
of the project and rebase is required. For example, while working on a feature branch, another team member might have
...
Similar to the previous method, the `-w`/`--web` can be used to open the report in the browser, see the changes in ERD
format, and more.

#### Compare against a specific branch

Users can instruct Atlas to detect what are the changes to analyze by comparing the current branch to a specific branch.
