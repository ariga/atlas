...
### Migration Hooks <LoginRequired /> {#migration-hooks}
...
or `lock_timeout` in PostgreSQL to prevent migrations from blocking other ongoing operations.

```hcl title="atlas.hcl" {1-10}
hook "sql" "timeout" {
  transaction {
    after_begin = [
...
revert schema changes made by specific migration files using the `atlas migrate down` command. For full details, see
the [down migration](/versioned/down) documentation.
