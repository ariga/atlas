...
3. **Push (Delivery)** - Use `atlas migrate push`, or set up a CI pipeline to push the latest migrations state to the
   [Atlas Cloud Schema Registry](/cloud/features/registry). Alternatively, you can package the migrations directory into
...
Users who have connected or pushed their migration directories to the Atlas Cloud Schema Registry can read the migrations state directly from
