...
:::note
This option is not recommended for general use. It is intended for use when you encounter limitations in the Atlas
schema definition or when your schema references objects that are not part of the desired state (schema) and you want
Atlas to ignore their existence.
:::

Atlas allows setting exclusion patterns for the migrations directory. Objects that match the exclusion patterns are not
considered during the `migrate diff` stage.
...
`migrate diff` stage - essentially, to ignore their existence.
...
For more examples, see the [exclude documentation](/declarative/inspect#exclude-schema-resources).
