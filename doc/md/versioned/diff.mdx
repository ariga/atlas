...
[HCL](/atlas-schema/hcl) or SQL schema definition, a database [URL](../concepts/url), external schemas like
[ORM](/atlas-schema/external).

### Summary

To get started with versioned migration, run `atlas migrate diff`. This command generates a new migration file that aligns
...
To learn more on how to configure Atlas to read the desired state of the schema from ORM definition, see the
...
By utilizing the [`atlas migrate lint`](/versioned/lint) command, teams can ensure that changes are added to the
migration directory in a sequential order to prevent unexpected behavior. Learn more on how to integrate Atlas into your
[**GitHub Actions**](/cloud/setup-ci) or [**GitLab**](/guides/ci-platforms/gitlab) CI pipelines.
:::
