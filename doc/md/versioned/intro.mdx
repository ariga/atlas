...
atlas migrate diff initial \
  --to file://schema.sql \
  --dev-url "clickhouse://localhost:9000" \
  --format '{{ sql . "  " }}'
```
...
```bash
docker run --rm -d --name atlas-demo -e CLICKHOUSE_DB=example -p 9001:9000 clickhouse/clickhouse-server:latest
```
...
# The "dev" environment represents our local testings.
env "local" {
  url = "clickhouse://localhost:9001/example"
  migration {
    dir = "atlas://app"
  }
}
