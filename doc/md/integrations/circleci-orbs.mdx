...
* `config` - The path to the Atlas configuration file.  By default, Atlas will look for a file
  ...
  Learn more about [Atlas configuration files](https://atlasgo.io/atlas-schema/projects).
* `env` - The environment to use from the Atlas configuration file.  For example, `dev`.
* `github_token_env` (required) - The GitHub token to use for authentication.
  Defaults to `GITHUB_TOKEN` environment variable.
* `github_repository_env` (required) - The GitHub repository name that will be used for reporting lint results.
