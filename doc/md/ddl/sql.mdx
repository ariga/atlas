...
Table partitioning refers to splitting logical large tables into smaller physical ones. Atlas currently supports
partitioned tables only in PostgreSQL and the rest of the dialects will be added in future versions.
...
disambiguated by using resource [qualifers](intro.md#qualifiers). The following document describes a
...
:::info
Note, it is recommended to use the [`--dev-url`](../dev-database) option when generated columns are used.
:::
...
:::info
Note, it is recommended to use the [`--dev-url`](../dev-database) option when index expressions are used.
:::
...
:::info
Note, it is recommended to use the [`--dev-url`](../dev-database) option when partial indexes are used.
:::
